...
describe('cell', () => {
	...
	it('returns a matrix of cell distances from a root cell', () => {
		const rootCell = new Cell(0, 0);
		const cellOne = new Cell(0, 1);
		const cellTwo = new Cell(0, 2);
		const cellThree = new Cell(1, 0);

		rootCell.link(cellOne);
		rootCell.link(cellThree);
		cellOne.link(cellTwo);

		const expectedDistances = new Distances(rootCell);

		expectedDistances.setCellDistance(cellOne, 1);
		expectedDistances.setCellDistance(cellTwo, 2);
		expectedDistances.setCellDistance(cellThree, 1);

		const result = rootCell.distances();

		expect(result.cells).toEqual(expectedDistances.cells);
	});
});
